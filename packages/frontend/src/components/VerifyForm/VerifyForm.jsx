    ...
    try {
      ...
      if (response.exists === false) {
        Notiflix.Notify.warning(
          "Email is not exists in the database. Please write a different email."
        );
        return;
      }

      let responseCheckVerify = await fetch(
        `https://avengers-wallet-app.onrender.com/api/users/checkVerify/${email}`
      );
      responseCheckVerify = await responseCheckVerify.json();

      if (response.exists && responseCheckVerify.verification) {
        Notiflix.Notify.warning(
          "Email is exists in the database and verified."
        );
        return;
      }

      let result = await fetch(
        "https://avengers-wallet-app.onrender.com/api/users/verify",
        {
          method: "post",
