...
const ResetPasswordForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let result = await fetch(
        "https://so-yummy-1f2e.onrender.com/api/users/forgot-password",
        {
          method: "post",
      ...
      if (result && email) {
        ...
        Notiflix.Notify.info(
          "A link to reset password to your account has been sent to the email address provided in the form."
        );
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <form onSubmit={handleSubmit} className={css.resetPasswordForm}>
      <Box
        sx={{ "& > :not(style)": { m: 2 } }}
        className={css.resetPasswordBox}
      >
